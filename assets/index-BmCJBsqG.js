import{d as E,g as A,r as c,j as e,aY as G,aZ as I,k as O,L as o,aW as R,Z as x,q as F,B as W,aj as $,N as s}from"./index-KPinFGVz.js";const B="var(--bg, #0d1117)",l="var(--text, #f3f4f6)",d="var(--muted, #a0a0a7)",t="var(--card, #111318)",r="var(--border, #23262d)",i="var(--accent, #5aa9ff)",C="var(--accent-soft, rgba(90,169,255,0.15))",h="var(--shadow, 0 10px 30px rgba(0,0,0,0.35))",H={Wrap:E.main`
    width: 100%;
    color: ${l};
    background: ${B};
  ...
  `},g=a=>String(a).padStart(2,"0"),M=a=>a.toDateString(),P=a=>`${g(a.getHours())}:${g(a.getMinutes())}:${g(a.getSeconds())}hrs`,b=a=>`${M(a)} ${P(a)}`,T=a=>{try{return typeof a=="string"?a:void 0}catch{return}},u=T("2026-09-25T00:53:18.755Z"),f=T("2026-09-21T22:46:10+05:30"),V=()=>{const{search:a}=A(),j=c.useMemo(()=>new URLSearchParams(a),[a]),S=j.get("email"),v=j.get("source"),y=S||"user@example.com",[z,k]=c.useState(!1),[Y,N]=c.useState(!1),w="vfy_DEMO_3f9c7b1c4",m=c.useMemo(()=>{const n=f||u,p=n?new Date(n):new Date;return{date:M(p),time:P(p),datetime:b(p)}},[]),L=async()=>{try{await navigator.clipboard.writeText(w),k(!0);const n=setTimeout(()=>k(!1),1800);return()=>clearTimeout(n)}catch{}},D=c.useCallback(()=>{N(!0),document.body.classList.add("print-section-mode");const n=()=>{document.body.classList.remove("print-section-mode"),N(!1),window.removeEventListener("afterprint",n)};window.addEventListener("afterprint",n),setTimeout(()=>window.print(),40)},[]);return c.useEffect(()=>{const n=document.getElementById("verify-title");if(!n)return;const p=requestAnimationFrame(()=>{try{n.focus({preventScroll:!0})}catch{n.focus()}});return()=>cancelAnimationFrame(p)},[]),e.jsxs(H.Wrap,{children:[e.jsx("style",{children:`
        @media print {
