import{d as G,u as U,r as t,j as e,N as f,aW as J,k as K,aT as Q,M as X,aX as Y,t as h,L as i,aU as y,p as Z}from"./index-KPinFGVz.js";const d="var(--text, #f3f4f6)",l="var(--muted, #a0a0a7)",p="var(--card, #111318)",o="var(--border, #23262d)",r="var(--accent, #5aa9ff)",v="var(--accent-soft, rgba(90,169,255,0.15))",w="var(--shadow, 0 10px 30px rgba(0,0,0,0.35))",_={Wrap:G.main`
        width: 100%;
        color: ${d};
    ...
    `};function x(a){return a<10?`0${a}`:`${a}`}function ee(a){const n=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],c=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];return`${n[a.getDay()]} ${c[a.getMonth()]} ${x(a.getDate())} ${a.getFullYear()}`}function se(a){return`${x(a.getHours())}:${x(a.getMinutes())}:${x(a.getSeconds())}hrs`}function u(a){return`${ee(a)} ${se(a)}`}const ie="2026-09-25T00:53:18.755Z",ae="2026-09-21T22:46:10+05:30";function oe(){const a=U(),[n,c]=t.useState("email"),[k,N]=t.useState(""),[$,C]=t.useState(""),[S,M]=t.useState(""),[P,T]=t.useState(""),[L,R]=t.useState(""),[D,g]=t.useState(!1),[te,F]=t.useState("info"),[O,B]=t.useState(""),[z,E]=t.useState(""),H=t.useMemo(()=>new Date,[]),A=t.useMemo(()=>new Date(ie),[]),I=t.useMemo(()=>new Date(ae),[]);t.useEffect(()=>{const s=document.querySelector("#reset-email, #reset-phone");if(s)try{s.focus({preventScroll:!0})}catch{s.focus()}},[n]);const q=t.useCallback(()=>{const s=document.body;s.classList.add("print-section-mode"),window.print();const b=()=>{s.classList.remove("print-section-mode"),window.removeEventListener("afterprint",b)};window.addEventListener("afterprint",b)},[]),j=t.useCallback(s=>{F(s==="success"?"success":"info"),B(s==="success"?"Password reset link dispatched":"Demo mode"),E(s==="success"?"For the demo, this simulates sending a secure reset link. In production, the link would expire and enforce device checks.":"This is a display-only build. Actions that modify data are disabled."),g(!0)},[]),W=s=>{s.preventDefault(),j("success")},V=s=>{s.preventDefault(),j("info")},m=()=>g(!1);return e.jsxs(_.Wrap,{children:[e.jsx("style",{children:`
        @media print {
